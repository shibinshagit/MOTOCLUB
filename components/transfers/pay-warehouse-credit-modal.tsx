...
import { payWarehouseCredit } from "@/app/actions/warehouse-payment-actions"
import type { WarehousePaymentAllocation } from "@/app/actions/warehouse-payment-actions"
import { useSelector } from "react-redux"
import type { RootState } from "@/store/store"

interface PayWarehouseCreditModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
  warehouse: {
...
export default function PayWarehouseCreditModal({
  isOpen,
  onClose,
  onSuccess,
  warehouse,
  ...
  const [paymentResult, setPaymentResult] = useState<{
    totalPaid: number
    allocations: WarehousePaymentAllocation[]
  ...
  const maxAmount = warehouse.we_owe
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true)
    try {
      const result = await payWarehouseCredit(
          ...
          <div className="flex flex-col max-h-[90vh]">
            <div className="bg-gradient-to-r from-green-600 to-emerald-600 text-white p-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-xl font-bold">Payment done</h2>
                  <p className="text-green-100 text-sm">Paid {paymentResult.warehouseName}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="rounded-lg bg-white/10 p-3 text-center">
                  <div className="text-xl font-bold">{formatCurrency(paymentResult.totalPaid)}</div>
                  <div className="text-xs text-green-100">Paid</div>
                </div>
                <div className="rounded-lg bg-white/10 p-3 text-center">
                  <div className="text-xl font-bold">{paymentResult.allocations.length}</div>
                  <div className="text-xs text-green-100">Transfers Updated</div>
                </div>
                <div className="rounded-lg bg-white/10 p-3 text-center">
                  <div className="text-xl font-bold">{formatCurrency(paymentResult.remainingCredit)}</div>
                  <div className="text-xs text-green-100">Left to pay</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
              {paymentResult.allocations.map((allocation) => (
                <div
                  key={allocation.transferId}
                  className="flex items-center justify-between rounded-lg border p-3 bg-gray-50"
                >
                  <div>
                    <div className="font-medium">Transfer #{allocation.transferId}</div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={allocation.newStatus === "paid" ? "default" : "secondary"}>
          ...
          <div className="flex flex-col max-h-[90vh]">
            <div className="bg-gradient-to-r from-indigo-600 to-blue-700 text-white p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold">Pay warehouse</h2>
                  <p className="text-blue-100 text-sm">Oldest unpaid transfers are paid first</p>
                </div>
              </div>
                ...
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500">Warehouse #{warehouse.warehouse_id}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-gray-500">Still to pay</div>
                    <div className="text-xl font-bold text-rose-600">{formatCurrency(maxAmount)}</div>
                  </div>
                </CardContent>
              </Card>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="warehouse-pay-amount">Amount *</Label>
                    <Input
                      id="warehouse-pay-amount"
                      type="number"
                      step="0.01"
                      min="0.01"
                      max={maxAmount}
                ...
                <div className="space-y-2">
                  <Label htmlFor="warehouse-pay-date">Payment Date (optional)</Label>
                  <Input
                    id="warehouse-pay-date"
                    type="date"
                    value={paymentDate}
                ...
                <div className="space-y-2">
                  <Label htmlFor="warehouse-pay-notes">Notes (optional)</Label>
                  <Textarea
                    id="warehouse-pay-notes"
                    value={notes}
