...
import {
  getWarehousePaymentById,
  updateWarehousePayment,
...
interface EditWarehousePaymentModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
...
export default function EditWarehousePaymentModal({
  isOpen,
  onClose,
  onSuccess,
  paymentId,
  userId,
  deviceId,
}: EditWarehousePaymentModalProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true)
      try {
        const result = await getWarehousePaymentById(paymentId)
        ...
        setWarehouseName(payment.warehouse_name)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true)
    try {
      const result = await updateWarehousePayment({
        paymentId,
        amount: parsedAmount,
        paymentMethod,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Edit payment{warehouseName ? ` — ${warehouseName}` : ""}</DialogTitle>
        </DialogHeader>
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="edit-wh-amount">Amount ({currency})</Label>
              <Input
                id="edit-wh-amount"
                type="number"
                step="0.01"
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-wh-date">Payment date</Label>
              <Input
                id="edit-wh-date"
                type="date"
                value={paymentDate}
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-wh-notes">Notes</Label>
              <Textarea
                id="edit-wh-notes"
                value={notes}
