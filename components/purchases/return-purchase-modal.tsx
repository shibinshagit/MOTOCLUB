...
import { processPurchaseReturn } from "@/app/actions/purchase-return-actions"
...
export default function ReturnPurchaseModal({
  ...
  useEffect(() => {
    if (isOpen && purchaseItems && purchaseItems.length > 0) {
      ...
      setRefundPaymentMethod(purchaseData?.payment_method || "Cash")
      setReason("")
      setStep("edit")
      setErrorMessage(null)
    }
  }, [isOpen, purchaseItems, purchaseData])
  ...
  const getRemainingReturnableQty = (item: any) => {
    ...
    return Math.max(0, purchasedQty - returnedQty)
  }
  ...
  const handleQuantityChange = (itemId: number, value: string, maxQty: number) => {
    ...
    if (!isRefundManuallyEdited) {
      ...
      purchaseItems.forEach((item) => {
        ...
        suggestedTotal += (qty * price) + ((qty * price) * ((Number(item.tax_percentage) || 0) / 100))
      })
  ...
  const calculateTotalReturnValue = () => {
    ...
    purchaseItems.forEach((item) => {
      ...
      const price = Number(item.price) || 0
      total += (qty * price) + ((qty * price) * ((Number(item.tax_percentage) || 0) / 100))
    })
    return total
  }
  ...
  const handleConfirmSubmit = async () => {
    if (!validateForm() || !purchaseId) return
    ...
    try {
      ...
      if (result.success) {
        notifySuccess(toast, result.message || "purchase return completed successfully", "Return Processed")
        onSuccess()
        onClose()
      } else {
        setErrorMessage(result.message || "Failed to process purchase return")
        setStep("edit")
      }
    } catch (err) {
  ...
  const returnedItemsList = purchaseItems.filter((item) => (returnQuantities[item.id] || 0) > 0)
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex flex-col items-end gap-1">
              <span className="rounded-md border border-slate-200 bg-white px-2.5 py-1 text-xs font-semibold text-slate-700">
                supplier: {purchaseData?.supplier_name || "Walk-in"}
              </span>
              <span className="text-xs font-semibold text-slate-700">
                supplier Paid Amount: <strong className="text-emerald-700 font-bold">{formatCurrency(purchaseData?.received_amount ?? purchaseData?.total_amount ?? 0)}</strong>
              </span>
            </div>
          </div>
        </DialogHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-xl border border-slate-200 bg-slate-50/50 p-4">
                <div className="space-y-3">
                  <div>
                    ...
                    <Select value={refundPaymentMethod} onValueChange={setRefundPaymentMethod}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="supplier Credit">supplier Account Balance</SelectItem>
                  ...
                  <div>
                    ...
                    <Input
                      placeholder="e.g. Defective product / supplier changed mind"
                      value={reason}
                  ...
                  <div className="space-y-1.5 border-b border-slate-100 pb-3">
                    <div className="flex justify-between text-xs text-slate-600">
                      ...
                      <span className="font-semibold text-slate-800">
                        {formatCurrency(purchaseData?.total_amount || 0)}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs text-slate-600">
                      <span>supplier Paid Amount:</span>
                      <span className="font-bold text-emerald-700">
                        {formatCurrency(purchaseData?.received_amount ?? purchaseData?.total_amount ?? 0)}
                      </span>
                    </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                <div className="rounded-lg border border-slate-200 bg-white p-3 text-center">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">supplier Paid</p>
                  <p className="mt-1 text-sm font-bold text-emerald-700">{formatCurrency(purchaseData?.received_amount ?? purchaseData?.total_amount ?? 0)}</p>
                </div>
                <div className="rounded-lg border border-slate-200 bg-white p-3 text-center">
