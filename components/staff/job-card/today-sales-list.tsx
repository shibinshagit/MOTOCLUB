  ...
  const handleDelete = async (saleId: number) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast({ title: "Error", description: res.message || "Failed to delete Job Card.", variant: "destructive" })
      }
    } catch (error) {
          ...
          <table className="min-w-full border-separate border-spacing-0 text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-[#F1F4F9] text-xs font-semibold uppercase tracking-wide text-slate-600">
                ...
                <th className="whitespace-nowrap px-4 py-2.5 text-left">Tracking Number</th>
