...
import { getFilteredPartnerOrders, getPartnerDashboardStats, getPartnerSalesAnalytics } from "@/app/actions/partner-actions"
...
import { PartnerGlobalDateFilter } from "@/components/partner/partner-global-date-filter"
...
export default async function PartnerDashboardPage({
  ...
  const dateRange = typeof rawParams.dateRange === "string" ? rawParams.dateRange : "This Month"
  ...
  const [salesResult, statsResult, analyticsResult] = await Promise.all([
    ...
    getPartnerDashboardStats(session.staffId, { dateRange, fromDate, toDate }),
    getPartnerSalesAnalytics(session.staffId, { dateRange, fromDate, toDate }),
  ])
  ...
  if (analyticsResult.success && analyticsResult.data) {
    chartSeries = analyticsResult.data
  }

  const periodLabel = dateRange === "Today" ? "Today's" : dateRange === "All" ? "All Time" : `${dateRange}'s`
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500 mt-1">
              Here's an overview of your partner account performance.
            </p>
          </div>
        </header>

        {/* Unified Global Date Filter */}
        <PartnerGlobalDateFilter />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="bg-gradient-to-br from-[#2979ff] to-[#1565c0] p-3.5 sm:p-4 rounded-xl shadow-md border-0 flex flex-col justify-between text-white relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <p className="mt-1 sm:mt-2 text-[10px] sm:text-[11px] font-medium text-blue-100">{periodLabel} Assigned</p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-[#00c853] to-[#009624] p-3.5 sm:p-4 rounded-xl shadow-md border-0 flex flex-col justify-between text-white relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <p className="mt-1 sm:mt-2 text-[10px] sm:text-[11px] font-medium text-emerald-100">In Selected Period</p>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-[#7c4dff] to-[#651fff] p-3.5 sm:p-4 rounded-xl shadow-md border-0 flex flex-col justify-between text-white relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start">
                <p className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-purple-100">Period Activity</p>
              ...
              <p className="mt-1 sm:mt-2 text-[10px] sm:text-[11px] font-medium text-purple-100">Orders Processed</p>
            </div>
          </div>
