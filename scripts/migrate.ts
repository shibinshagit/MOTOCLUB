  ...
  await run("products", async () => {
    await sql`
      CREATE TABLE IF NOT EXISTS products (
        ...
        own_ecom_status VARCHAR(20) DEFAULT 'active',
...
async function upgradeLegacyColumns() {
  ...
  const columns: Array<[string, () => Promise<any>]> = [
    ...
    ["devices.admin_pages", () => sql`ALTER TABLE devices ADD COLUMN IF NOT EXISTS admin_pages TEXT`],
    ...
    ["products.own_ecom_status", () => sql`ALTER TABLE products ADD COLUMN IF NOT EXISTS own_ecom_status VARCHAR(20) DEFAULT 'active'`],
    ["products.own_ecom_status_default", () => sql`ALTER TABLE products ALTER COLUMN own_ecom_status SET DEFAULT 'active'`],
    ["products.own_ecom_status_backfill", () => sql`UPDATE products SET own_ecom_status = 'active' WHERE own_ecom_status IS NULL OR own_ecom_status = 'not_listed'`],
    ...
    ["sales.advance_amount", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS advance_amount DECIMAL(12,2) DEFAULT 0`],
    ["sales.balance_amount", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS balance_amount DECIMAL(12,2) DEFAULT 0`],
    ["sales.payment_status", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS payment_status VARCHAR(50) DEFAULT 'Pending'`],
    ...
    ["purchases.courier_charge", () => sql`ALTER TABLE purchases ADD COLUMN IF NOT EXISTS courier_charge DECIMAL(12,2) DEFAULT 0`],
    ["purchases.discount", () => sql`ALTER TABLE purchases ADD COLUMN IF NOT EXISTS discount DECIMAL(12,2) DEFAULT 0`],
    ...
    ["sales.customer_name_override", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS customer_name_override VARCHAR(255)`],
    ["sales.customer_phone_override", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS customer_phone_override VARCHAR(50)`],
    ["sales.customer_address", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS customer_address TEXT`],
    ["sales.shipping_city", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS shipping_city VARCHAR(255)`],
    ["sales.shipping_street", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS shipping_street TEXT`],
    ["sales.shipping_landmark", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS shipping_landmark VARCHAR(255)`],
    ["sales.shipping_address_type", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS shipping_address_type VARCHAR(50)`],
    ["sales.shipping_pincode", () => sql`ALTER TABLE sales ADD COLUMN IF NOT EXISTS shipping_pincode VARCHAR(20)`],
    ["customers.city", () => sql`ALTER TABLE customers ADD COLUMN IF NOT EXISTS city VARCHAR(255)`],
    ["customers.street", () => sql`ALTER TABLE customers ADD COLUMN IF NOT EXISTS street TEXT`],
